...
class SWAGInference(object):
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            k = self.update_count + 1
            self.mu_swag[name] = (self.mu_swag[name] * self.update_count + param) / k
            self.sq_mu_swag[name] = (self.sq_mu_swag[name] * self.update_count + param*param) / k
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_xs,) in loader:
                pred = torch.softmax(self.network(batch_xs), dim = -1)
        ...
        bma_probabilities = torch.zeros(per_model_sample_predictions[0].shape)
        for predictions in per_model_sample_predictions:
            bma_probabilities += predictions
        bma_probabilities /= len(per_model_sample_predictions)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_std = torch.nan_to_num(torch.sqrt(self.diag_covariance[name]))
